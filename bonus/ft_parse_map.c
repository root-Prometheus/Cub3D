...
/*   Updated: 2023/10/26 15:59:23 by rbattal          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	ft_check_map_elem(char c, int *content)
{
	if (ft_strchr(" 0123NSEWC", c) == 0)
		return (printf("Error. Invalid map: Invalid component\n"), 0);
	else if (c == PLAYER_N
		|| c == PLAYER_E
		|| c == PLAYER_S
		|| c == PLAYER_W)
...
static int	ft_check_map(t_map *map)
{
	...
	if (content > 1)
		return (printf("Error. Invalid map: Too many players\n"), 0);
	else if (content < 1)
		return (printf("Error. Invalid map: Missing player\n"), 0);
	return (1);
}
